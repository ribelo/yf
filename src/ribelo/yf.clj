...
(defn share-statistics [symbol]
  (when (stock? symbol)
    {:avg-vol-3-month                     (avg-vol-3-month symbol)
     ...
     :float                               (float symbol)
     :percent-hold-by-insiders            (percent-held-by-insiders symbol)
     :percent-hold-by-institutions        (percent-held-by-institutions symbol)
  ...
  (when (stock? symbol)
    (->> (get-profile-htree symbol)
         ...
         ((fn [[street city country]]
            {:street  (str/lower-case street)
             :city    (str/lower-case city)
             :country (str/lower-case country)})))))
